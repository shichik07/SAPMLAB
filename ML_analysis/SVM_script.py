    ...
    if ind[part] != 1:
       ...
       define the parameter space that will be searched over
        ...
        for kernel in ('linear', 'rbf'):
            ...
            # Location to save my output
            if kernel == 'linear':
                save_p = os.getcwd()[:-4]+'Results\\' + file[part][:-10] +'_linR_results.csv'
            else:
                save_p = os.getcwd()[:-4]+'Results\\' + file[part][:-10] +'_rbfK_results.csv'
            kernel = 'linear'  
            ...
            while os.path.exists(save_p):
                cnt += 1
                if kernel == 'linear':
                    save_p = os.getcwd()[:-4]+'Results\\' + file[part][:-10] + '_v' + str(cnt) + '_linR_results.csv'
                else:
                    save_p = os.getcwd()[:-4]+'Results\\' + file[part][:-10] + '_v' + str(cnt) + '_rbfK_results.csv'
